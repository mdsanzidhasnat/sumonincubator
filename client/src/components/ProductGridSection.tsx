...
export const ProductGridSection: React.FC<ProductGridSectionProps> = ({
  ...
  return (
    <section id="all-products" className="py-12 bg-slate-50 border-b border-bismillah-borderLight">
      <div className="max-w-7xl mx-auto px-4 space-y-8">
        {/* SECTION HEADER WITH TABS & SORT */}
        <SectionHeaderBlock
          title={lang === 'bn' ? 'আমাদের সকল প্রোডাক্টস' : 'OUR ALL PRODUCTS'}
          subtitle={
            lang === 'bn'
        ...
        >
          {!isPreview && (
            <>
              <div className="flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar bg-white p-1 rounded-sharp border border-bismillah-borderLight">
                {filterTabs.map((tab) => (
                  <button
                    key={tab.id}
                    onClick={() => setSelectedCategory(tab.id)}
                    className={`px-3.5 py-1.5 rounded-sharp text-xs font-bold transition-colors cursor-pointer whitespace-nowrap ${
              ...
              <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-sharp border border-bismillah-borderLight text-xs font-semibold text-slate-700">
          ...
          {isPreview && viewAllLink && (
            <Link
              to={viewAllLink}
              className="inline-flex items-center gap-2 bg-bismillah-primaryGreen hover:bg-emerald-700 text-white font-bold text-sm px-6 py-2 rounded-t-sharp transition-colors"
            >
        ...
        {displayed.length > 0 ? (
          ...
          <div className="bg-white rounded-sharp p-12 text-center border border-bismillah-borderLight space-y-3">
            ...
            <button
              ...
              className="bg-bismillah-primaryGreen text-white font-bold text-xs px-4 py-2 rounded-sharp"
            >
